...
if page == "🏠 Home & Summary":
    ...
    **Key Update Performed:**
    ...
    with tab1:
        ...
        # === TOTALS FOR ORIGINAL PICK REPORT ===
        ...
        total_dpmo_pick = df_pick_orig["DPMO"].sum()   # Note: Sum of DPMO is not very meaningful, but included as requested
        
        col1, col2, col3, col4 = st.columns(4)
        with col1:
            st.metric("Total Users", total_users_pick)
        with col2:
            st.metric("Total Opportunities", f"{total_opp_pick:,}")
        with col3:
            st.metric("Total Defects", total_defects_pick)
        with col4:
            st.metric("Total DPMO (Sum)", f"{total_dpmo_pick:,}")
    ...
    with tab1:
        ...
        # === TOTALS FOR ORIGINAL STOW REPORT ===
        ...
        col1, col2, col3, col4 = st.columns(4)
        with col1:
            st.metric("Total Users", total_users_stow)
        with col2:
            st.metric("Total Opportunities", f"{total_opp_stow:,}")
        with col3:
            st.metric("Total Defects", total_defects_stow)
        with col4:
            st.metric("Total DPMO (Sum)", f"{total_dpmo_stow:,}")
