...
# ====================== UPDATED RATIOS & NEW COLUMNS ======================
nar_value_per_hour = 50.75
nar_cost_per_hour = 19.00
nar_hours_worked = 30.29
nar_total_opp = 1814
nar_equiv = 2.97

# Calculations for all associates
df_spend["Value per 32h (Ratio)"] = (df_spend["Estimated Value"] / nar_value_per_hour * 32).round(2)
df_spend["Value per Hour"] = f"${nar_value_per_hour:.2f}"
df_spend["Cost per Hour"] = f"${nar_cost_per_hour:.2f}"
df_spend["Total Weekly Hours Worked"] = (df_spend["Estimated Value"] / nar_value_per_hour).round(2)

# New Columns
df_spend["Total Hours Scheduled"] = ""
df_spend["PTO Use"] = ""

mask = df_spend["User"] == "narossoh"
df_spend.loc[mask, "Total Hours Scheduled"] = "36"
df_spend.loc[mask, "PTO Use"] = "5.08"

# Format Value per 32h
...
df_spend = df_spend[[
    ...
    "Value per Hour", "Cost per Hour", "Total Weekly Hours Worked",
    "Total Hours Scheduled", "PTO Use", "Equivalent Associates"
]]
...
if page == "🏠 Home & Summary":
    ...
    # ==================== UPDATED KEY METRICS ====================
    st.subheader("🔑 Narossoh Key Metrics")
    col1, col2, col3, col4, col5, col6 = st.columns(6)
    with col1:
        st.metric("Hours Worked", "30.29")
    with col2:
        st.metric("Hours Scheduled", "36")
    with col3:
        st.metric("PTO Used", "5.08")
    with col4:
        st.metric("Cost/Hour", "$19.00")
    with col5:
        st.metric("Value/Hour", "$50.75")
    with col6:
        st.metric("Equivalent Associates", "2.97×")
    ...
    st.info("""
    **Narrossoh Summary**  
    • Actual Hours Worked: **30.29**  
    • Scheduled: **36** hrs | PTO: **5.08** hrs  
    • Cost Rate: **$19/hr** | Value Rate: **$50.75/hr**
    """)
