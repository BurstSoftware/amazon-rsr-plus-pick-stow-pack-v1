import streamlit as st
import pandas as pd
import plotly.express as px

# Page configuration
st.set_page_config(
    page_title="Pick Pack Stow App",
    page_icon="📦",
    layout="wide",
    initial_sidebar_state="expanded"
)

st.title("🧳 Pick • Pack • Stow")
st.markdown("### Warehouse Performance Tracker")

# Sidebar navigation
page = st.sidebar.selectbox(
    "Select Page",
    ["📋 Overview", "📊 Performance DataFrame", "👤 Associate Dashboards", "📈 Interactive Performance Graph"]
)

# ====================== UPDATED DATA ======================
data = {
    "Name": ["David", "Elizet", "Giselle", "Omar", "Jennifer", "Nathan"],
    "Hours Worked": [40.00, 40.00, 40.00, 40.00, 40.00, 30.29],
    "Stow Quantity": [57, 204, 580, 0, 127, 1068],
    "Pick Quantity": [614, 176, 362, 55, 804, 746],
    "PC99 Errors": [1, 1, 0, 0, 0, 1],
    "SIPS Over Short": [0, 1, 1, 0, 4, 19],
    "Scan Out of Sequence": [0, 3, 9, 0, 0, 79],
    "Bin Collisions": [2, 7, 5, 0, 3, 65],
    "Wrong ASINs": [13, 6, 7, 2, 14, 57]
}

df = pd.DataFrame(data)

# Cost calculations
hourly_rates = [22.50, 22.50, 22.50, 22.50, 22.50, 19.00]
df["Hourly Rate ($)"] = hourly_rates
df["Total Units"] = df["Stow Quantity"] + df["Pick Quantity"]
df["Total Cost ($)"] = df["Hourly Rate ($)"] * df["Hours Worked"]
df["Avg Cost per Unit ($)"] = df.apply(
    lambda row: round(row["Total Cost ($)"] / row["Total Units"], 4) 
    if row["Total Units"] > 0 else 0.0, 
    axis=1
)

df["Total Cost ($)"] = df["Total Cost ($)"].round(2)
df["Hourly Rate ($)"] = df["Hourly Rate ($)"].round(2)

# ====================== PAGE 1: OVERVIEW ======================
if page == "📋 Overview":
    st.header("Manager Overview")
    st.write("""**Pick Pack Stow App includes 5 managers that work 40 hours a week.** 
    Their names are David, and David stows 57 packages with 1 pc99 error and 2 bin colisions, 
    and picks 614 units, with 13 wrong asin, elizet worked 40 hours, stowed 204 packages, 
    with 1 pc99 error, 1 sips over short, 3 scan out of sequence and 7 bin collisions, 
    elizet picks 176 units, with 6 wrong asins, Giselle worked 40 hours, stowed 580 units, 
    with 1 sip over short, 9 scan out of sequence and 5 bin collisions, she picks 362 units 
    with 7 wrong asins, Omar works 40 hours, stows 0 units, picks 55 units, with 2 wrong asins, 
    Jennifer works 40 hours, stows 127 units, with 4 sips over short and 3 bin colisions, 
    **Jennifer picked 804 units with 14 wrong asins** and Nathan works 30.29 hours, 
    stows 1068 units, with 1 pc99, 19 sips over short, 79 scan out out of sequence, 
    and 65 bin colisions, **Nathan picked 746 units with 57 wrong asins**.""")
    
    col1, col2, col3, col4 = st.columns(4)
    with col1: st.metric("Total Team Members", "6")
    with col2: st.metric("Full-time Managers", "5")
    with col3: st.metric("Part-time", "1 (Nathan)")
    with col4: st.metric("Total Hours Worked", "230.29")

# ====================== PAGE 2: DATAFRAME ======================
elif page == "📊 Performance DataFrame":
    st.header("📊 Manager Performance DataFrame")
    st.info("**Cost basis:** Managers = $22.50/hr | Nathan = $19.00/hr")
    
    column_order = ["Name", "Hours Worked", "Stow Quantity", "Pick Quantity", "Total Units",
                    "Hourly Rate ($)", "Total Cost ($)", "Avg Cost per Unit ($)",
                    "PC99 Errors", "SIPS Over Short", "Scan Out of Sequence", 
                    "Bin Collisions", "Wrong ASINs"]
    
    st.dataframe(df[column_order], use_container_width=True, height=420,
                 column_config={
                     "Hourly Rate ($)": st.column_config.NumberColumn(format="$.2f"),
                     "Total Cost ($)": st.column_config.NumberColumn(format="$.2f"),
                     "Avg Cost per Unit ($)": st.column_config.NumberColumn(format="$.4f"),
                 })

# ====================== PAGE 3: ASSOCIATE DASHBOARDS ======================
elif page == "👤 Associate Dashboards":
    st.header("👤 Individual Associate Dashboards")
    for _, row in df.iterrows():
        with st.container(border=True):
            st.subheader(f"📌 {row['Name']}")
            c1, c2, c3 = st.columns(3, gap="large")
            with c1: st.metric("Labor Cost", f"${row['Total Cost ($)']:,.2f}")
            with c2: st.metric("Total Units Handled", f"{int(row['Total Units']):,}")
            with c3: st.metric("Avg Cost per Unit", f"${row['Avg Cost per Unit ($)']:.4f}")
            st.divider()

# ====================== PAGE 4: INTERACTIVE GRAPH ======================
else:
    st.header("📈 Interactive Performance Graph")
    st.markdown("**Click on any bar** to see detailed Pick / Stow stats for that associate.")

    # Bar chart - Total Units
    fig = px.bar(
        df,
        x="Name",
        y="Total Units",
        text="Total Units",
        color="Total Units",
        color_continuous_scale="Blues",
        title="Total Units Handled by Each Associate (Click a bar for details)"
    )
    fig.update_traces(textposition="outside", marker_line_color="black", marker_line_width=1)
    fig.update_layout(height=500, xaxis_title="Associate", yaxis_title="Total Units",
                      title_font_size=20)

    # Display chart with click support
    selected = st.plotly_chart(fig, use_container_width=True, on_select="rerun")

    # Get clicked point
    if selected and selected["selection"]["points"]:
        clicked_name = selected["selection"]["points"][0]["x"]
        st.success(f"**Selected: {clicked_name}**")
        
        # Show detailed stats
        person = df[df["Name"] == clicked_name].iloc[0]
        
        col1, col2 = st.columns([1, 1])
        with col1:
            st.subheader("📦 Stow & Pick Stats")
            st.metric("Stow Quantity", f"{person['Stow Quantity']:,}")
            st.metric("Pick Quantity", f"{person['Pick Quantity']:,}")
            st.metric("**Total Units**", f"{person['Total Units']:,}")
        
        with col2:
            st.subheader("💰 Cost Metrics")
            st.metric("Labor Cost", f"${person['Total Cost ($)']:,.2f}")
            st.metric("Avg Cost per Unit", f"${person['Avg Cost per Unit ($)']:.4f}")
            st.metric("Hourly Rate", f"${person['Hourly Rate ($)']:.2f}")
        
        st.subheader("⚠️ Error Metrics")
        err_col1, err_col2, err_col3, err_col4 = st.columns(4)
        with err_col1: st.metric("PC99 Errors", person["PC99 Errors"])
        with err_col2: st.metric("SIPS Over/Short", person["SIPS Over Short"])
        with err_col3: st.metric("Scan Out of Sequence", person["Scan Out of Sequence"])
        with err_col4: st.metric("Bin Collisions", person["Bin Collisions"])
        st.metric("Wrong ASINs", person["Wrong ASINs"])
        
    else:
        st.info("👆 Click on any bar in the chart above to view detailed Pick, Stow, and Cost statistics.")

# Footer
st.sidebar.markdown("---")
st.sidebar.caption("Built with Python 3.14.4 + Streamlit + Plotly")
st.sidebar.caption("Pick • Pack • Stow Performance App")
