...
# Pack Data = Exact mirror of Pick
...
df_pack_orig = pd.DataFrame(pack_data)
df_stow_orig = pd.DataFrame(stow_data)

# ====================== NEW: Combined Per-User Total Volume ======================
df_combined = df_pick_orig[["User", "Opportunities"]].copy()
df_combined = df_combined.rename(columns={"Opportunities": "Pick"})

df_pack_temp = df_pack_orig[["User", "Opportunities"]].rename(columns={"Opportunities": "Pack"})
df_stow_temp = df_stow_orig[["User", "Opportunities"]].rename(columns={"Opportunities": "Stow"})

df_combined = df_combined.merge(df_pack_temp, on="User", how="left")
df_combined = df_combined.merge(df_stow_temp, on="User", how="left")
df_combined = df_combined.fillna(0)
df_combined["Total Units"] = df_combined["Pick"] + df_combined["Pack"] + df_combined["Stow"]
df_combined = df_combined.sort_values("Total Units", ascending=False).reset_index(drop=True)
...
if page == "🏠 Home & Summary":
    ...
    **Key Update Performed:**
    ...
    with tab5:
        st.subheader("👤 Per User Total Volume (Pick + Pack + Stow)")
        st.info("**This table shows the combined contribution of each associate across all three processes.**")

        # Styled DataFrame
        styled_combined = df_combined.style.format({
            "Pick": "{:,.0f}",
            "Pack": "{:,.0f}",
            "Stow": "{:,.0f}",
            "Total Units": "{:,.0f}"
        }).background_gradient(subset=["Total Units"], cmap="Blues")

        st.dataframe(styled_combined, use_container_width=True, hide_index=True)

        # Top 10 Chart
        st.subheader("Top 10 Associates by Total Volume")
        top10 = df_combined.head(10)
        top_chart = alt.Chart(top10).mark_bar().encode(
            x=alt.X("Total Units:Q", title="Total Units"),
            y=alt.Y("User:N", sort="-x", title="Associate"),
            color=alt.Color("Total Units:Q", scale=alt.Scale(scheme="blues")),
            tooltip=["User", "Pick", "Pack", "Stow", "Total Units"]
        ).properties(height=400)
        st.altair_chart(top_chart, use_container_width=True)

    st.info("""
    **Notes**:
    - Pack volume = Pick volume for every user.
    - Total Units = Pick + Pack + Stow per associate.
    - This gives the most accurate view of each associate’s overall contribution.
    """)
...
# ====================== REMAINING PAGES (unchanged) ======================
    ...
    # Keep your original code for this page (copy from previous version)
    st.title("⏰ Associate Work Hours & Productivity")
    # ... (paste your full original code for this page here if needed)

# Add the rest of your original pages (Team Overview, Payroll, etc.) here as they were.
# For brevity, they are unchanged and can be copied from your previous file.
