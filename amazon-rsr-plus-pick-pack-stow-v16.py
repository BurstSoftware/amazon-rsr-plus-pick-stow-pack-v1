...
# ====================== NEW: TOTAL VOLUME BASED VALUE DATA (from uploaded image) ======================
volume_data = {
    "User": ["narossoh", "gpliegom", "stajenni", "iqrayuss", "matstrak", "nkaibrah", "mnimhas", 
             "arrizola", "hasnsai", "uiyps", "mtiband r", "elizev", "danijac", "hersmary", 
...
page = st.sidebar.radio("Go to:", 
    ["🏠 Home & Summary", 
     ...
     "📊 Narossoh Volume Equivalence Report"])   # ← New page added
...
if page == "🏠 Home & Summary":
    ...
    **Note**: 
    ...
    st.markdown("**April 5th – April 12th, 2026** | Spending Allocation by Associate")

    total_spend = 9554.00

    pick_opp = df_pick_orig.set_index("User")["Opportunities"]
    stow_opp = df_stow_orig.set_index("User")["Opportunities"]
    ...
    combined["Estimated Spend"] = (combined["% of Total Volume"] / 100 * total_spend).round(2)

    combined = combined.sort_values("Total Opportunities", ascending=False)

    st.subheader(f"Total Spend Allocation: ${total_spend:,.2f}")
    st.caption("Based on each associate's share of total opportunities (Pick + Stow)")
    ...
    st.subheader("Spending Distribution by Associate")
    chart_data = display_df.head(10)

    spend_chart = alt.Chart(chart_data).mark_bar().encode(
        x=alt.X("Associate:N", sort="-y"),
        y=alt.Y("Estimated Spend:Q", title="Estimated Spend ($)"),
        tooltip=["Associate", "Total Opportunities", "% of Total Volume", "Estimated Spend"]
    ...
    st.info("""
    **Notes**: 
    - Total spend of $9,554.00 is allocated based on each associate's share of total opportunities (Pick + Stow).
    - This shows how much of the total budget is attributable to each associate's volume during April 5th–12th.
    """)
    ...
    st.info("""
    **Data Source**: Direct from weekly spend report.  
    Total spend of **$9,554** is distributed proportionally based on each associate’s share of total opportunities (Pick + Stow).  
    **narossoh** has the highest volume and therefore the largest estimated spend (~17%).
    """)
    ...
    with col3:
        st.metric("**Total Time (Pack + Trickle)**", f"{combined_time_hours:.2f} hours", 
                  delta=f"{combined_time_hours:.1f} hrs")
    ...
    **To process {total_packages:,} packages at narossoh's current rate:**
    ...
    st.caption("""
    Note: This assumes narossoh works continuously at the observed rate. 
    Real-world factors like breaks, fatigue, setup time, or team distribution are not included.
    """)
    ...
    **Narrossoh performs the work of approximately {equiv_associates} average associates.**
    ...
    His total volume (Pick + Stow) is equivalent to the combined output of about **{equiv_associates}** typical team members.
    """)

    st.markdown("---")
    st.subheader("Associates Ranked by Total Volume (Descending)")

    ranked = df_volume.sort_values("Total Opportunities", ascending=False).reset_index(drop=True)
    ...
    st.caption("Data Source: Total Volume Based Value Report • Narossoh is a clear top performer in total workload.")
