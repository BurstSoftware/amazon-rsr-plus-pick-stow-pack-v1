...
# ====================== DATA & COST CALCULATIONS (AVAILABLE TO ALL PAGES) ======================
data = {
...
# ====================== PAGE 3: NEW UI — INDIVIDUAL ASSOCIATE DASHBOARDS ======================
else:
    st.header("👤 Individual Associate Dashboards")
    st.markdown("**Labor Cost, Total Units Handled & Average Cost per Unit** for each manager and Nate")
    
    st.info("""
    **Cost basis:**  
    Managers (David, Elizet, Giselle, Omar, Jennifer) = $22.50/hour  
    Nathan (Nate) = $19.00/hour  
    Labor Cost = Hourly Rate × Hours Worked  
    Avg Cost per Unit = Labor Cost ÷ Total Units Handled
    """)
    
    # Create a clean card-style UI for each associate
        ...
        with st.container():
            ...
            with c3:
                st.metric(
                    ...
                    value=f"${row['Avg Cost per Unit ($)']:.4f}",
                    delta=None
                )
...
st.sidebar.caption("Individual Associate Cost UI added")
